...
npm metadata, station app, Tauri shell, and docs site. Use the release script to
update all version files together, create the release commit, and create the
annotated Git tag:
...
- `apps/openipc-station/src-tauri/tauri.conf.json`
- `docs/package.json`
- `docs/package-lock.json`

To update files without committing, or to commit without tagging:

```sh
scripts/release-version.sh 0.2.0 --no-commit
scripts/release-version.sh 0.2.0 --no-tag
```

To push both the current branch and tag:
...
crates.io packages, publish `@openipc-rs/web` to npm, and upload Tauri desktop
